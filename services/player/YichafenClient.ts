...
import { isCli as isCliEnv } from '../../infrastructure/platform/core/ProcessDetector';
...
export class YichafenClient {
  ...
  /**
   * @param remoteBase - 远程资源服务器基础 URL，默认 https://bot.weiqi.lol
   */
  constructor(remoteBase: string = DEFAULT_REMOTE_BASE) {
  ...
  async query(
    ...
    try {
      ...
      console.warn("[YichafenClient] query ERROR:", error);
  ...
  private async loadPlayers(network: NetworkManager, timeout: number): Promise<PlayerData[]> {
    ...
    if (isCli) {
      ...
      for (const url of getSnapshotCandidates(3)) {
        const filename = url.split('/').pop() + '.gz';
        const remoteUrl = REMOTE_BASE + '/' + encodeURIComponent(filename);
        try {
          const response = await network.request<ArrayBuffer>({
            url: remoteUrl,
            method: 'GET',
          ...
          if (response?.data) {
            ...
            console.info('[YichafenClient] Remote data loaded:', remoteUrl);
            return data;
          }
        } catch (e) {
          console.info('[YichafenClient] Remote load failed:', remoteUrl, e);
        }
      }
      return [];
    }
    ...
    console.info('[YichafenClient] Web/App branch, isApp:', isApp);
