...
export interface ManagementServiceConfig {
  ...
  versionUrl?: string;
  /** 远程资源服务器基础 URL，默认 https://bot.weiqi.lol */
  remoteBase?: string;
}
...
export class ManagementService implements IManagementService {
  ...
  constructor(config: ManagementServiceConfig = {}) {
    ...
    this.remoteBase = config.remoteBase ?? DEFAULT_REMOTE_BASE;
  }
  ...
  /**
   * 获取远程版本信息
   * App 环境下通过本地代理 /proxy 请求远程 version.json，绕过 AssetServer 缓存
   * Web 环境下直接请求（与 getVersion 相同）
   */
  async getRemoteVersion(): Promise<VersionInfo> {
    try {
      const isApp = this.isAppEnvironment();
      let remoteUrl: string;

      if (isApp) {
        // App 环境：通过 /proxy 请求远程，绕过 AssetServer 本地缓存
        // 先获取远程 version.json 的绝对 URL
        const versionUrl = await this.getRemoteVersionUrl();
        remoteUrl = '/proxy?url=' + encodeURIComponent(versionUrl);
      } else {
        // Web 环境：直接请求（无本地缓存问题）
        remoteUrl = await this.getVersionUrl();
      }
      ...
      console.error('[ManagementService] Failed to get remote version:', error);
      return {
        version: '未知',
      };
    }
  ...
  /**
   * 获取远程 version.json 的绝对 URL
   * App 环境下从桥接配置获取，Web 环境下无需此方法
   */
  private async getRemoteVersionUrl(): Promise<string> {
    // 优先使用直接指定的 URL
    if (this.versionUrlOverride) {
      // 如果是相对路径，转为绝对路径
      if (!this.versionUrlOverride.startsWith('http')) {
        return buildRemoteUrl(this.remoteBase, this.versionUrlOverride);
      }
      return this.versionUrlOverride;
    }
    ...
    if (this.configProvider) {
      const config = await this.configProvider.getModuleConfig<IManagementConfig>('management');
      if (config.versionUrl) {
        if (!config.versionUrl.startsWith('http')) {
          return buildRemoteUrl(this.remoteBase, config.versionUrl);
        }
        return config.versionUrl;
      }
    }

    // 默认：{remoteBase}/version.json
    return buildRemoteUrl(this.remoteBase, '/version.json');
  }
