...
export class FoxwqShareProvider extends BaseProvider {
  ...
  private readonly jueyiLiveProvider?: FoxwqJueyiLiveProvider;
  ...
  constructor(
    ...
    if (snifferProvider) {
      this.jueyiLiveProvider = new FoxwqJueyiLiveProvider(network, snifferProvider);
    }
  }
  ...
  async fetch(url: string): Promise<FetchResult> {
    ...
    try {
      ...
      // 步骤 1：尝试 API 模式（历史棋谱）
      // 注意：直播棋谱也含 chessid，但 API 返回的 SGF 可能不准确（如第一手重复）
      // 直播棋谱应走步骤3的 Sniffer/Protobuf 路径，跳过 API
      const isLive = this.isLiveUrl(url);
      if (chessId && !isLive) {
      ...
      // 步骤 3：尝试 Sniffer 模式（直播棋谱，优先走 Protobuf 解码）
      if (this.jueyiLiveProvider && this.isLiveUrl(url)) {
        const liveResult = await this.jueyiLiveProvider.fetch(url);
        if (liveResult.success) {
          timing.total = this.now() - startTime;
          liveResult.timing = { ...liveResult.timing, ...timing };
          return liveResult;
        }
        // Sniffer 失败，回退到 API（虽然 API 返回的 SGF 可能有小问题，但总比没有好）
        if (chessId) {
          const apiResult = await this.tryFetchViaApi(chessId, url, timing, startTime);
          if (apiResult) {
            return apiResult;
