...
export class GameFoxwqHelper {
  ...
  private createMetadata(chessid: string, sgfContent: string): FetchResult['metadata'] {
    ...
    try {
      ...
      const haMatch = sgfContent.match(/HA\[([^\]]*)\]/);
      ...
      if (szMatch) {
        ...
        if (sz.includes(':')) {
          ...
          width = parseInt(parts[0]!, 10) || 19;
          height = parseInt(parts[1]!, 10) || 19;
        } else {
          const size = parseInt(sz, 10) || 19;
          width = size;
          height = size;
        }
      }
      if (kmMatch) komi = parseFloat(kmMatch[1]!) || 6.5;
      if (haMatch) handicap = parseInt(haMatch[1]!, 10) || 0;
    } catch (e) {
      // 解析失败，保持默认值
    }
