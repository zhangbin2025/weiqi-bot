...
export interface IGameService {
   ...
   * @param source - 棋谱来源（'foxwq' | 'katago'，默认 'foxwq'）
   * @returns 棋谱 URL 列表
   */
   ...
   * @returns SGF 条目列表
   */
  fetchKatagoGames(date: string, limit?: number): Promise<KatagoSgfEntry[]>;

  /**
   * 获取 KataGo Archive 可用日期列表
   * @param count - 最多返回几个日期
   * @returns 日期条目列表
   */
  listKatagoArchiveDates(count?: number): Promise<Array<{ date: string; url: string }>>;
}
