...
export class GameService implements IGameService {
   ...
   */
  async fetchKatagoGames(date: string, limit?: number): Promise<KatagoSgfEntry[]> {
    const katago = this.registry.getKatagoProvider();
    return katago.fetchGamesByDate(date, limit);
  }

  /**
   * 获取 KataGo Archive 可用日期列表
   */
  async listKatagoArchiveDates(count?: number): Promise<Array<{ date: string; url: string }>> {
    const katago = this.registry.getKatagoProvider();
    return katago.listArchiveDates(count);
  }
