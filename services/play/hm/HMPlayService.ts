...
export class HMPlayService implements IHMPlayService {
  ...
  /**
   * 替玩家生成着法（用于自动落子测试）
   * 使用服务层维护的 previousBoard，正确处理打劫
   */
  async genmoveForAutoPlay(): Promise<{ x: number; y: number; winRate: number; scoreLead: number } | null> {
    const state = this.game.getState();
    const moves = this.getMoveHistory();
    const handicapStones = this.game.getHandicapStones();
    const initialStones = handicapStones.map(s => ({
    ...
    return this.aiController.genmove(
      getBoardState(state.board),
      this.previousBoard, // 使用服务层维护的 previousBoard，正确处理打劫
      state.currentPlayer,
      moves,
      state.komi,
      visits,
