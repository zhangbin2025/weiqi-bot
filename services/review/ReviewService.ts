 ...
 * @fileoverview 复盘服务
 */
import { normalizeKomi } from '../../domain/game/KomiNormalizer';
...
export class ReviewService implements IReviewService {
  ...
  async loadFromSGF(sgf: string): Promise<string> {
    ...
    // 解析贴目（使用 domain 层 normalizeKomi 统一处理）
   ...
   * 批量评估指定着法的胜率（直播增量着法用）
   * @param reviewId 复盘 ID
   * @param moveIndices 要评估的着法索引数组
   * @returns 每手的胜率和目差
   */
  async evaluateMovesBatch(reviewId: string, moveIndices: number[]): Promise<Array<{ moveNumber: number; winRate: number; scoreLead: number }>> {
    const data = this.getReview(reviewId);
    this.ensureAI();
    const komi = data.komi ?? 7.5;

    // App 端：用原生 analyzeGame（batch MCTS），一次请求分析所有增量着法
    const engine = (this.ai as any)?.engine as IAIEngine | undefined;
    if (engine?.analyzeGame) {
      const moves = data.moves.slice(0, Math.max(...moveIndices) + 1).map(m => ({
        player: m.color as PlayerColor,
        x: m.x,
        y: m.y,
      }));
      ...
      return results.map((r: any, i: number) => ({
        ...
        winRate: r.winRate ?? r.rootWinRate ?? 0.5,
        scoreLead: r.scoreLead ?? r.rootScoreLead ?? 0,
      }));
    }
    ...
    return moveIndices.map((idx, i) => ({
      moveNumber: idx + 1,
      winRate: evals[i]!.winRate,
