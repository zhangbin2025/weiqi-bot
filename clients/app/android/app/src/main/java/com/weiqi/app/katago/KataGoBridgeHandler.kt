    ...
    private fun handleDownloadModel(
        ...
        currentDownloadJob = lifecycleScope.launch(Dispatchers.IO) {
            ...
            try {
                ...
                val request = okhttp3.Request.Builder().url(url).header("Accept-Encoding", "identity").build()  // 禁止透明解压，保存原始 gzip 文件
                ...
                if (e is kotlinx.coroutines.CancellationException) {
                    Logger.i(TAG, "Download cancelled (superseded by new download)")
                } else {
