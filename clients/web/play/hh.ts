...
async function main() {
  ...
  // ===== AI 自动落子（测试功能）=====
  // 通过编译选项开启：VITE_AI_AUTO_PLAY=true vite dev
  // 默认不开启，生产构建不包含此代码
  declare const __ENABLE_AI_AUTO_PLAY__: boolean;
  if (__ENABLE_AI_AUTO_PLAY__) {
    let autoPlayRunning = false;

    // 使用轮询检测轮次，避免覆盖页面回调导致棋盘不刷新
    setInterval(async () => {
      if (autoPlayRunning) return;
      const s = hhPlayApp.getState();
      const myColor = s.me?.color;
      if (!myColor || s.gameEnded || s.currentPlayer !== myColor) return;
      if (!s.inGame && s.moveHistory.length === 0) return; // 还没开局
      if (!aiController.isInitialized()) return;

      autoPlayRunning = true;
      try {
        // IHHPlayState.moveHistory 会过滤掉 pass，无法可靠 replay previousBoard
        // 因此不传 previousBoard，打劫判断由 canMove() 兜底
        const result = await aiController.genmove(
          s.board, null, s.currentPlayer,
          s.moveHistory.map(m => ({ x: m.x, y: m.y, player: m.color })),
          7.5, 10
        );
        if (result && hhPlayApp.canMove(result.x, result.y)) {
          hhPlayApp.move(result.x, result.y);
        } else {
          // genmove 返回 null 或禁着点，停一手
          hhPlayApp.pass();
        }
      } catch (e) {
        // genmove 异常（如 board 状态不一致），停一手
        hhPlayApp.pass();
      } finally {
    ...
    console.info("[HH-AI] AI 自动落子模式已启用（测试）");
  }
