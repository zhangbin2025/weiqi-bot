...
async function main() {
  ...
  if (tunnelManager.isClientMode()) {
    console.info('[hm.ts] 客户端模式，后台启动隧道连接');
  ...
  if (__ENABLE_AI_AUTO_PLAY__) {
    ...
    setInterval(async () => {
      ...
      if (!hmPlayApp.isPlayerTurn() || hmPlayApp.isEnded()) return;
      if (!aiController.isInitialized()) return;

      autoPlayRunning = true;
      try {
        const result = await hmPlayApp.genmoveForAutoPlay();
        if (result && result.x !== -1 && result.y !== -1) {
          const ok = await hmPlayApp.playerMove(result.x, result.y);
          if (!ok) {
            failCount++;
            if (failCount >= 3) {
              await hmPlayApp.playerPass();
              failCount = 0;
            }
          } else {
            failCount = 0;
    ...
    console.info('[HM-AI] AI auto-play enabled (test)');
  }
}
