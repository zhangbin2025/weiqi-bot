...
export class AssistantPage {
  ...
  constructor() {
    ...
    this.managementService = new ManagementService({ remoteBase: this.remoteBase }); // 创建管理服务
  ...
  /**
   * 初始化远程服务器基础 URL
   * App 环境下从原生桥接获取，Web 环境下使用默认值
   */
  private async initRemoteBase(): Promise<void> {
    const isApp = typeof navigator !== 'undefined' && navigator.userAgent.includes('WeiqiApp');
    if (isApp) {
      try {
        const configResponse = await (window as any).prompt('config:get');
        if (configResponse) {
          const config = JSON.parse(configResponse);
          if (config.remoteBase) {
            this.remoteBase = config.remoteBase;
            // 更新已创建的 ManagementService 和 CommandHandler
            (this.managementService as any).remoteBase = this.remoteBase;
            (this.useCase as any).commandHandler.remoteBase = this.remoteBase;
          }
        }
      } catch (e) {
        console.warn('[AssistantPage] Failed to get remoteBase from bridge, using default:', e);
      }
    }
  }
  ...
  async init(): Promise<void> {
    // App 环境：从桥接获取 remoteBase
    await this.initRemoteBase();
    // 初始化 SessionService
  ...
  private async checkForUpdate(): Promise<void> {
    ...
    try {
      const [localVersion, remoteVersion] = await Promise.all([
        this.managementService.getVersion(),
        this.managementService.getRemoteVersion(),
      ]);
      ...
      // 静默失败，不影响正常使用
      console.warn('[AssistantPage] 版本检查失败:', error);
    }
  }
