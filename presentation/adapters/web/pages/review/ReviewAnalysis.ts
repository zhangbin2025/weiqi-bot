...
export class ReviewAnalysis {
  ...
  /** 更新当前复盘 ID（用于直播增量更新后重新创建 reviewId） */
  ...
  async getAnalysisVisits(): Promise<number> {
    // 胜率图统一 visits=1，Web adapter 层转 0 走 batch
    return 1;
  }
  ...
  async startAnalysis(mode: 'quick' | 'deep' = 'quick', taskId?: string, baseMoves?: Array<{ x: number; y: number; color: PlayerColor }>): Promise<void> {
    ...
    try {
      ...
      // 胜率图描绘：visits=1，Web 端转为 0 走 batch 批量评估；App 端保持 1 走原生 MCTS
      const isApp = typeof navigator !== 'undefined' && navigator.userAgent.includes('WeiqiApp');
      const visits = isApp ? 1 : 0;
  ...
  /** 销毁当前 reviewId 并清除状态(用于重置棋谱) */
  destroyCurrent(): void {
    if (this.reviewId) {
      console.info("[ReviewAnalysis] 销毁旧 reviewId:", this.reviewId);
