...
export interface VariationState {
  ...
  /** 候选选点表格 HTML（recommendation 模式下保存，退出恢复用） */
...
export interface InteractionCallbacks {
  ...
  /** 直播模式下点击推荐圆圈 → 跳转复盘页面 */
  onLiveCircleClick?: (circle: RecommendationCircle) => void;
  /** 恢复候选选点表格（退出状态栈时恢复上一层表格内容） */
  onRestoreCandidates?: (html: string) => void;
}
...
export class ReviewInteraction {
  ...
  handleBoardClick(x: number, y: number): void {
    // 只读模式（直播模式）：点击推荐圆圈时跳转复盘，其他点击拦截
    if (this.callbacks.isReadOnlyMode?.()) {
      const clickedCircle = this.board.getClickedRecommendation(x, y);
      if (clickedCircle) {
        this.callbacks.onLiveCircleClick?.(clickedCircle);
      }
      return;
    }
    // 没有加载棋谱时不允许试下
  ...
  private pushCurrentState(): void {
    ...
    this.stateStack.push({
      ...
      candidatesHtml: this.getCandidatesHtml(),
    });
