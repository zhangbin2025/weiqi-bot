...
export class JosekiDiscoverPage implements IPage {
  ...
  private currentSource: DiscoverSource = 'foxwq';
   ...
   * 执行定式发现（支持后台任务）
   */
  async executeDiscover(limit: number, taskId?: string, source?: string): Promise<void> {
    if (this.analyzing) return;
    this.analyzing = true;

    try {
      const discoverSource = (source || 'foxwq') as DiscoverSource;
      this.uiHelper.showProgress(true);
      this.uiHelper.updateProgress(0, '正在获取棋谱列表...');
