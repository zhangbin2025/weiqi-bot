...
export interface RecommendationCircle {
  ...
  nextColor?: PlayerColor; // 下一着的颜色（用于圆圈颜色）
}
/** 棋盘图片渲染器 */
export class BoardImageRenderer {
  ...
  drawRecommendationCircle(
    ...
    if (nextColor === 'black') {
      fillColor = `rgba(80, 80, 80, ${alpha * 0.5})`;
      textColor = `rgba(255, 255, 255, ${alpha})`;
    } else if (nextColor === 'white') {
      fillColor = `rgba(230, 230, 230, ${alpha * 0.85})`;
      textColor = `rgba(30, 30, 30, ${alpha})`;
    } else {
      // 未指定颜色时默认按黑棋处理
      fillColor = `rgba(80, 80, 80, ${alpha * 0.5})`;
      textColor = `rgba(255, 255, 255, ${alpha})`;
