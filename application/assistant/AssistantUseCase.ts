...
export class AssistantUseCase implements IAssistantUseCase {
  ...
  constructor(config: AssistantUseCaseConfig) {
    ...
    this.commandHandler = new CommandHandler({
      ...
      remoteBase: config.remoteBase ?? DEFAULT_REMOTE_BASE,
  ...
  async sendMessage(text: string): Promise<void> {
    ...
    // 判断是否精确匹配（输入完全等于核心关键词或变体关键词）
    const isExactMatch = !!(recognizeResult.matchedKeywords && recognizeResult.matchedKeywords.length > 0);
