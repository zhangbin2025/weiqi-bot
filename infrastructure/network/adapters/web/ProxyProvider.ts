...
export class ProxyProvider extends BaseProvider {
  ...
  private buildProxyUrl(originalUrl: string): string {
    ...
    if (originalUrl.startsWith('/') || originalUrl.startsWith('.') || !originalUrl.includes('://')) {
      const base = (typeof window !== "undefined" && window.location) ? window.location.origin + window.location.pathname : "http://localhost/"; const absoluteUrl = new URL(originalUrl, base).toString();
