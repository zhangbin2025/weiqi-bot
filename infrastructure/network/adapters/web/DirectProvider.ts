...
export class DirectProvider extends BaseProvider {
  ...
  /**
   * 带进度回调的 Blob 读取（支持进度驱动的 stall 超时）
   *
   * 与 requestWithTimeout 的固定超时不同，这里使用 stall 超时：
   * 每次收到数据就重置计时器，只有连续 stallTimeoutMs 无新数据才超时。
   * 这确保大文件在慢网络下只要有进度就不会被误杀。
   *
   * @param response - Fetch 响应
   * @param config - 请求配置（包含 expectedSize 和 onProgress）
   * @param stallTimeoutMs - 进度停滞超时（毫秒），默认 30 秒
   */
  private async readBlobWithProgress(
    response: Response,
    config: IRequestConfig,
    stallTimeoutMs: number = 30_000
  ): Promise<Blob> {
    ...
    const resetStallTimer = () => {
      ...
      stallTimer = setTimeout(() => {
        stallReject?.(new Error(`Download stalled: no progress for ${stallTimeoutMs}ms`));
      }, stallTimeoutMs);
    };
