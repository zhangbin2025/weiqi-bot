...
export class DirectProvider extends BaseProvider {
  ...
  async request<T>(config: IRequestConfig): Promise<IResponse<T>> {
    ...
    // file:// protocol support for CLI environment
    if (url.startsWith('file://')) {
      ...
      try {
        ...
        switch (responseType) {
          case 'arraybuffer':
            data = buffer.buffer.slice(buffer.byteOffset, buffer.byteOffset + buffer.byteLength) as T;
            break;
          case 'text':
        ...
        throw new NetworkError(
          e instanceof Error && (e as NodeJS.ErrnoException).code === 'ENOENT' ? `File not found: ${filePath}` : (e instanceof Error ? e.message : String(e)),
          'FILE_ERROR',
          this.name,
          config
        );
