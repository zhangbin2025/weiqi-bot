...
export class KataGoAppAdapter implements IAIEngine {
   ...
   */
  constructor(private networkManager?: NetworkManager, private remoteBase: string = DEFAULT_REMOTE_BASE) {}
  ...
  async init(options: AIEngineInitOptions): Promise<void> {
    ...
    if (isExternalUrl) {
      ...
      downloadUrl = `${this.remoteBase}/${normalizedPath}`;
    ...
    if (!canSkipDownload) {
      ...
      // 进度驱动的 stall 超时：每次收到进度回调都重置
      // 默认 30 秒无进度才超时（而不是固定 5 分钟总超时）
      ...
      const resetStallTimeout = () => {
        ...
        stallTimeoutId = setTimeout(() => {
          downloadReject?.(new Error(`Download stalled: no progress for ${DOWNLOAD_STALL_TIMEOUT_MS / 1000}s`));
        }, DOWNLOAD_STALL_TIMEOUT_MS);
      };

      resetStallTimeout();

      // 设置下载进度回调（按 filename 过滤，避免旧下载的进度干扰）
      if (options.onProgress) {
        this.client.onDownloadProgress = (info) => {
          // 只处理当前模型的进度，忽略旧下载的残留进度
          if (info.filename !== filename) {
            console.log(`[KataGoAppAdapter] Ignoring stale progress for: ${info.filename} (expecting: ${filename})`);
            return;
          }
      ...
      this.client.onDownloadComplete = (info) => {
        // 只处理当前模型的完成事件，忽略旧下载的残留事件
        if (info.filename && info.filename !== filename) {
          console.log(`[KataGoAppAdapter] Ignoring stale downloadComplete for: ${info.filename} (expecting: ${filename})`);
          return;
        }
